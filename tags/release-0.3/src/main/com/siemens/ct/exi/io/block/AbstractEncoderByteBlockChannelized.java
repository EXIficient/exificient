...
public abstract class AbstractEncoderByteBlockChannelized extends
	...
	public AbstractEncoderByteBlockChannelized(OutputStream outputStream,
	...
	protected EncoderChannel getValueChannel(String uri, String localName) {
		...
		if (valueChannels.containsKey(uri)) {
			Map<String, EncoderChannelChannelized> valueChannelsLocalNamePerURI = valueChannels
					.get(uri);
			...
			if (!valueChannelsLocalNamePerURI.containsKey(localName)) {
				...
				valueChannelsLocalNamePerURI.put(localName,
						createEncoderChannel());
	...
	public void flush() throws IOException {
		assert (orderValueChannelUri.size() == orderValueChannelLocalName
				.size());
		/*
		...
		if (getNumberOfBlockValues() <= CompileConfiguration.MAX_NUMBER_OF_VALUES) {
			...
			for (int i = 0; i < orderValueChannelUri.size(); i++) {
				singleStream.write(valueChannels.get(
						orderValueChannelUri.get(i)).get(
		 ...
		 */
			...
			for (int i = 0; i < orderValueChannelUri.size(); i++) {
				EncoderChannelChannelized channelLEQ100 = valueChannels.get(
						orderValueChannelUri.get(i)).get(
				...
				if (channelLEQ100.getNumberOfChannelValues() <= CompileConfiguration.MAX_NUMBER_OF_VALUES) {
					...
					// orderValueChannelLocalName.get ( i ) + " --> "
					// +channelLEQ100.getNumberOfChannelValues ( ) );
				}
			}
			...
			for (int i = 0; i < orderValueChannelUri.size(); i++) {
				EncoderChannelChannelized channelGre100 = valueChannels.get(
						orderValueChannelUri.get(i)).get(
						orderValueChannelLocalName.get(i));
				if (channelGre100.getNumberOfChannelValues() > CompileConfiguration.MAX_NUMBER_OF_VALUES) {
					...
					// orderValueChannelLocalName.get ( i ) + " --> "
					// +channelGre100.getNumberOfChannelValues ( ) );
				}
			}
		}
