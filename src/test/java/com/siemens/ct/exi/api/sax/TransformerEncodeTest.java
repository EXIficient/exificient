...
public class TransformerEncodeTest {
    ...
    public void testEncode() throws Exception {
        ...
        final Document decode = decode(factory, new InputSource(new ByteArrayInputStream(bytes)));
        final Diff diff = XMLUnit.compareXML(XMLUnit.buildControlDocument(XML), decode);
    ...
    public void testEncodeTransformer() throws Exception {
        final StringReader reader = new StringReader(XML);
        final EXIFactory factory = getFactory(true);
        final byte[] bytes = transformerEncode(factory, new InputSource(reader)).toByteArray();
        final Document decode = decode(factory, new InputSource(new ByteArrayInputStream(bytes)));
        final Diff diff = XMLUnit.compareXML(XMLUnit.buildControlDocument(XML), decode);
        Assert.assertTrue(diff.toString(), diff.similar());
    }

    private static EXIFactory getFactory(final boolean preservePrefixes) throws UnsupportedOption {
    ...
    private static ByteArrayOutputStream encode(final EXIFactory exiFactory, final StringReader in) throws Exception {
    ...
    private static Document decode(final EXIFactory exiFactory, final InputSource is) throws Exception {
