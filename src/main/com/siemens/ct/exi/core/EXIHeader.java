...
public class EXIHeader
{
	...
	public static void parse ( BitDecoderChannel headerChannel ) throws EXIException
	{
		try
		{
			// An EXI header starts with Distinguishing Bits part, which is a two
			// bit field 1 0
			if ( headerChannel.decodeNBitUnsignedInteger ( NUMBER_OF_DISTINGUISHING_BITS ) != DISTINGUISHING_BITS_VALUE )
			{
			...
			// The first bit of the version field indicates whether the version is a
			...
			// 3. If the value is 15, go to step 1, otherwise (i.e. the value being
			...
			do
			{
				value = headerChannel.decodeNBitUnsignedInteger ( NUMBER_OF_FORMAT_VERSION_BITS );
				version += value;
