...
public abstract class AbstractEncoderChannel implements EncoderChannel
{
	...
	public void encodeInteger ( int n ) throws IOException
	{
		...
		{
			encodeBoolean ( false );
			encodeUnsignedInteger ( Math.abs ( n ) );
		}
	}
	...
	public void encodeDecimal ( XSDDecimal decimal ) throws IOException, RuntimeException
	{
	...
	public void encodeDateTime ( Calendar cal, DatetimeType type ) throws IOException
	{
		switch ( type )
		{
			...
			case date: // date Year, MonthDay, [TimeZone]
				...
				encodeNBitUnsignedInteger ( XSDDatetime.getMonthDay ( cal ), XSDDatetime.NUMBER_BITS_MONTHDAY );
			...
			case dateTime: // dateTime Year, MonthDay, Time, [FractionalSecs],
				...
				encodeInteger ( cal.get ( Calendar.YEAR ) - XSDDatetime.YEAR_OFFSET );
				encodeNBitUnsignedInteger ( XSDDatetime.getMonthDay ( cal ), XSDDatetime.NUMBER_BITS_MONTHDAY );
				encodeNBitUnsignedInteger ( XSDDatetime.getTime ( cal ), XSDDatetime.NUMBER_BITS_TIME );
				encodeDateTimeFractionalSecs ( XSDDatetime.getFractionalSecondsReverse ( cal
			...
			case time: // time Time, [FractionalSecs], [TimeZone]
				this.encodeNBitUnsignedInteger ( XSDDatetime.getTime ( cal ), XSDDatetime.NUMBER_BITS_TIME );
				encodeDateTimeFractionalSecs ( XSDDatetime.getFractionalSecondsReverse ( cal
						.get ( Calendar.MILLISECOND ) ) );
				encodeDateTimeTimezone ( XSDDatetime.getTimeZoneInMinutesOffset ( cal ) );
				break;
	...
	public void encodeDateTime ( XSDDatetime datetime ) throws IOException
	{
		switch ( datetime.getDatetimeType ( ) )
		{
			...
			case date: // date Year, MonthDay, [TimeZone]
				encodeInteger ( datetime.iYear );
				encodeNBitUnsignedInteger ( datetime.iMonthDay, XSDDatetime.NUMBER_BITS_MONTHDAY );
				encodeDateTimeTimezone ( datetime.iTZMinutes );
				break;
			case dateTime: // dateTime Year, MonthDay, Time, [FractionalSecs],
				// [TimeZone]
				encodeInteger ( datetime.iYear );
				encodeNBitUnsignedInteger ( datetime.iMonthDay, XSDDatetime.NUMBER_BITS_MONTHDAY );
				encodeNBitUnsignedInteger ( datetime.iTime, XSDDatetime.NUMBER_BITS_TIME );
			...
			case gDay: // gDay MonthDay, [TimeZone]
				...
				break;
			case time: // time Time, [FractionalSecs], [TimeZone]
				this.encodeNBitUnsignedInteger ( datetime.iTime, XSDDatetime.NUMBER_BITS_TIME );
				encodeDateTimeFractionalSecs ( datetime.iFractionalSecs );
				encodeDateTimeTimezone ( datetime.iTZMinutes );
				break;
			default:
	...
	private void encodeDateTimeTimezone ( int tzMinutes ) throws IOException
	{
		...
		{
			encodeBoolean ( true );
			encodeNBitUnsignedInteger ( tzMinutes, XSDDatetime.NUMBER_BITS_TIMEZONE );
		}
		else
		{
			encodeBoolean ( false );
