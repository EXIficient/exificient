...
import com.siemens.ct.exi.datatype.DatatypeNBitLong;
...
public class NBitLongDatatypeEncoder extends AbstractDatatypeEncoder
		implements DatatypeEncoder {
	private int valueToEncode;
	private int numberOfBits;

	public NBitLongDatatypeEncoder(TypeEncoder typeEncoder) {
		super(typeEncoder);
	}
	...
	public boolean isValid(Datatype datatype, String value) {
		try {
			...
			assert (datatype instanceof DatatypeNBitLong);
			DatatypeNBitLong nBitDT = (DatatypeNBitLong) datatype;

			// check lower & upper bound
			if (lValueToEncode >= nBitDT.getLowerBound()
					&& lValueToEncode <= nBitDT.getUpperBound()) {
				// retrieve offset & update value
				lValueToEncode -= nBitDT.getLowerBound();
